...
function cardSvg(setup: string, punchline: string, petUrl: string, roast: string, template?: string) {
  ...
  const hSvg = hLines.map((l,i) => '<text x="524" y="'+(hY+i*55)+'" text-anchor="middle" fill="white" font-size="42" font-weight="900" font-family="system-ui">'+esc(l)+'</text>').join("\n");
  const pY = petUrl ? 1250 : 1300;
  const pSvg = pLines.map((l,i) => '<text x="524" y="'+(pY+i*36)+'" text-anchor="middle" fill="rgba(255,255,255,0.85)" font-size="24" font-family="system-ui">'+esc(l)+'</text>').join("\n");
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/api/card" || url.pathname === "/api/card-image") {
      const t = url.searchParams.get("template") || "pet_standup";
      const r = url.searchParams.get("roast") || "roast";
      const p = url.searchParams.get("pet") || "";
      return new Response(cardSvg(url.searchParams.get("setup")||"", url.searchParams.get("punchline")||"", p, r, t), {headers:{"Content-Type":"image/svg+xml","Cache-Control":"public, max-age=3600"}});
    }
    if (url.pathname === "/api/create-card") {
      const b = await request.json() as any;
      const tid = b.template_id || selectTemplate(b.facts||[], b.occasion||"x");
      const cs = generateJokes(tid, b.recipient_name||"Your recipient", b.facts||[], b.occasion||"x", b.roast_level||"roast", b.pet_name, b.pet_species, 5);
      return Response.json({template:tid,top_candidate:cs[0],other_candidates:cs.slice(1),preview:{svg:cardSvg(cs[0].setup,cs[0].punchline,b.pet_image_url,b.roast_level||"roast",tid)},quote:{unit_cost:3.50,currency:"GBP",shipping_cost:2.99}});
    }
    if (url.pathname === "/api/templates") return Response.json({templates});
    if (url.pathname === "/api/quote") return Response.json({unit_cost:3.50,currency:"GBP",shipping_cost:2.99});
    if (url.pathname === "/api/order") return Response.json({order_id:"sandbox_"+crypto.randomUUID().slice(0,8),status:"pending"});
    // Serve index.html from assets
    const resp = await env.ASSETS.fetch(new URL("/index.html", request.url));
